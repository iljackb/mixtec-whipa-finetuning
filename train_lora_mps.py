...
def main():
    ...
    ap.add_argument("--resume-from-checkpoint", type=str, default=None,
                     help="Resume training from a saved checkpoint instead of starting "
                          "over. Pass 'auto' to resume from the most recent "
                          "checkpoint-<step> folder found in --output-dir (requires that "
                          "a previous run into this same --output-dir was interrupted, "
                          "not completed -- a completed run overwrites checkpoint dirs "
                          "with the final merged model), or pass an explicit path to a "
                          "specific checkpoint-<step> folder. Omit entirely to train from "
                          "scratch (the default, unchanged behavior).")
    args = ap.parse_args()
    ...
    # True (auto-detect the latest checkpoint-<step> dir under output_dir),
    # an explicit path string, or None (train from scratch).
    resume = None
    ...
    print("\n" + "=" * 80)
    if resume:
        print(f"Resuming fine-tuning from checkpoint: "
              f"{'most recent in ' + args.output_dir if resume is True else resume}")
    else:
