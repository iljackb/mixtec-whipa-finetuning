...
- **Combining multiple extraction scripts' output into one manifest.** WhIPA has no concept of assembling training data from more than one source format at all. This is likely to be a necessary step as most corpora (especially for low-resource language/language documentation projects) are likely to be from a number of different sources.  `extract_finetune_data_unified.py`  performs this merge internally by classifying and dispatching each `<u>` independently across the whole corpus in a single pass, so there's no longer a separate combining step. (Note if accessed before 2026/09/21 this was originally handled by a separate `combine_manifests.py` script, since retired)
