...
- **Extraction of token-level orthography, gold IPA, and audio timestamps from the source corpus format** (in this case TEI/XML, but the underlying gap, no ingestion path for anything other than WhIPA's own benchmark corpora, applies to any custom format). This project's source corpus has three structurally different formats needing separate handling: [extract_finetune_data.py](https://github.com/iljackb/mixtec-whipa-finetuning/blob/main/extract_finetune_data.py) for single-word-per-utterance files, [extract_finetune_data_sentences.py](https://github.com/iljackb/mixtec-whipa-finetuning/blob/main/extract_finetune_data_sentences.py) for multi-word-per-utterance files with real word-level timing, and [extract_finetune_data_myuc.py](https://github.com/iljackb/mixtec-whipa-finetuning/blob/main/extract_finetune_data_myuc.py) for whole-utterance-only sources with no word-level timing at all (extracting one training example per utterance rather than per word).
...
- **Combining multiple extraction scripts' output into one manifest.** WhIPA has no concept of assembling training data from more than one source format at all. Handled by [combine_manifests.py](https://github.com/iljackb/mixtec-whipa-finetuning/blob/main/combine_manifests.py), which accepts any number of already-extracted CSVs from any folder and merges them, reading each row's own self-assigned `source_corpus` label rather than needing to be told what each input file is.
